...
import co.roverlabs.sdk.Factory;
...
public class CardListAdapter extends RecyclerView.Adapter<CardListAdapter.CardViewHolder> {
    ...
    public void onBindViewHolder(final CardViewHolder holder, int position) {
        ...
        final ViewModel listView = card.getListView();

        if(!card.hasBeenViewed()) {
            //RoverEventBus.getInstance().post(new CardViewedEvent(mVisitId, card.getId()));
        }
        ...
        for (Block block : blocks) {
            ...
            switch (block.getType()) {
                ...
                case Constants.VIEW_BLOCK_TYPE_TEXT:
                    ...
                    UiUtils.setText(Constants.VIEW_BLOCK_TYPE_TEXT, holder.cardTextContentLayout, block.getTextContent(), block.getTextBlockStyles(mContext));
                    break;

                case Constants.VIEW_BLOCK_TYPE_IMAGE:
                ...
                case Constants.VIEW_BLOCK_TYPE_BUTTON:
                    ...
                    UiUtils.setText(Constants.VIEW_BLOCK_TYPE_BUTTON, holder.cardButton, block.getButtonLabel(), block.getLabelTextStyle(mContext));
                ...
                case Constants.VIEW_BLOCK_TYPE_BARCODE:
                    ...
                    if(block.getBarcodeFormat().equals(Constants.BARCODE_FORMAT_PLU)) {
                        barcodeLabelStyle = block.getPluTextStyle(mContext);
                        holder.cardBarcode128.setVisibility(View.GONE);
                    }
                    else if(block.getBarcodeFormat().equals(Constants.BARCODE_FORMAT_128)) {
                        barcodeLabelStyle = block.getLabelTextStyle(mContext);
                        int barcodeWidth = UiUtils.getDeviceWidthInPx(mContext);
                        int barcodeHeight = UiUtils.convertDpToPx(mContext, Constants.BARCODE_HEIGHT);
                    ...
                    UiUtils.setText(Constants.VIEW_BLOCK_TYPE_BARCODE, holder.cardBarcodeLabel, block.getBarcodeLabel(), barcodeLabelStyle);
            ...
            if(blockLayout != null && blockUrl != null) {
                ...
                blockLayout.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        //RoverEventBus.getInstance().post(new CardClickedEvent(mVisitId, card.getId(), blockUrl));
                        ...
                        if (blockScheme != null) {
                            if(blockScheme.equals(Constants.URL_SCHEME_ROVER)) {
//                                ((CardListActivity)mActivity).setShouldStartService(false);
//
//                                ViewModel detailView = card.getDetailView(blockUriString.substring(blockUriString.lastIndexOf("/") + 1));
//                                intent.setClass(mContext, CardDetailActivity.class);
//                                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
//                                intent.putExtra(Constants.VIEW_TYPE_DETAIL, new Gson().toJson(detailView));
...
//        ViewModel view = card.getListView();
